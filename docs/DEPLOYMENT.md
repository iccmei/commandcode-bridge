...
## One-command user systemd install/uninstall
...
Remove the service and installed files while preserving credentials/env:

```bash
./uninstall.sh
```

Remove credentials/env too:
...
| `COMMANDCODE_UPSTREAM_MODE`        | `auto`                       | `auto` probes the Provider API at startup and uses it when the plan allows (Provider $15/mo or higher); `provider` forces it; `alpha` forces the legacy `/alpha/generate` tunnel for every model. |
