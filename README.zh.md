...
CommandCode Bridge 是面向 CommandCode 账号的可信环境 HTTP 网关。它提供标准 OpenAI-compatible 模型与聊天端点，在符合条件的上游凭据之间路由请求，并发布与 CommandCode **1.53.0** 对齐的准确 **70-model** catalog。Bridge 版本始终跟随当前 CommandCode CLI 版本并在其后附加字母后缀（例如 **1.53.0.a**）；后缀表示仅限 Bridge 的发布。
...
| DeepSeek          | `deepseek/deepseek-v4.1-flash`          | 1,000,000 |    否    |
